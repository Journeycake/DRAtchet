...
# dratchet-server — client/, store/, and app/ aren't in dratchet-server's
# own dependency graph, but their manifests still have to exist here (see
# client/Cargo.toml, store/Cargo.toml, and app/Cargo.toml below).
...
RUN mkdir -p core/src server/src client/src store/src app/src \
    && echo "fn main() {}" > server/src/main.rs \
    && echo "fn main() {}" > client/src/main.rs \
    && touch core/src/lib.rs server/src/lib.rs client/src/lib.rs store/src/lib.rs app/src/lib.rs \
    && cargo build --release -p dratchet-server \
    && rm -rf core/src server/src client/src store/src app/src
...
COPY store/Cargo.toml store/Cargo.toml
COPY app/Cargo.toml app/Cargo.toml
RUN mkdir -p client/src store/src app/src \
    && echo "fn main() {}" > client/src/main.rs \
    && touch client/src/lib.rs store/src/lib.rs app/src/lib.rs \
