//! Message records, stored per conversation. Storage only in this
//! sub-phase: the disappearing-message *sweep* that actually acts on
//! `expires_at` is Phase 1.5.3.
...
pub struct Message {
    ...
    /// eligible for the Phase 1.5.3 sweep once `t` has passed.
...
impl Db {
    ...
    pub fn list_messages(&self, conversation_id: [u8; 16]) -> Result<Vec<Message>> {
        let mut messages: Vec<Message> = self
            .keys_with_prefix(&message_key_prefix(conversation_id))?
            .into_iter()
            .map(|key| {
                let bytes = self
                    .get_encrypted(&key)?
